...
def generate_and_save_trajectories_arcc(
    model_path: str,
    adapter_path: str,
    data_path: str,
    temperature: float,
    ...
    """
    Step 1: Generate TARPO model trajectories on ARC-Challenge, extract soft/hard token
    ...
    print(f"Loading ARC-Challenge dataset from: {data_path}")
...
def evaluate_saved_trajectories_arcc(jsonl_path: str, k: int, output_dir: str):
    ...
    confidence intervals, and soft-token ratio statistics for ARC-Challenge.
    """
    ...
    print("\n" + "=" * 50)
    print("TARPO ARC-CHALLENGE EVALUATION SUMMARY")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="data/arcc/arcc.json", help="Path to ARC-Challenge JSON")
    ...
    eval_dir = f"{checkpoint_path}/eval_arcc_k{args.k}_batch{args.batch_size}_{max_new_tokens}"
    os.makedirs(eval_dir, exist_ok=True)
    jsonl_file = os.path.join(eval_dir, "raw_trajectories.jsonl")

    print("\n" + ">" * 15 + " STEP 1: GENERATION " + "<" * 15)
    generate_and_save_trajectories_arcc(
