...
def generate_and_save_trajectories(
    ...
    """
    Step 1: Generate TARPO model trajectories on OlympiadBench, extract soft/hard token
    ...
    print(f"Loading OlympiadBench dataset from: {data_path}")
    ...
    with open(output_jsonl_path, 'a', encoding='utf-8') as f_out:
        for i in range(existing_count, total_samples, batch_size):
            ...
            true_answers = [item["final_answer"] for item in batch_data]
            ...
            for j in range(current_batch_size):
                ...
                true_answer = process_math_answer(str(true_answers[j]))
...
def evaluate_saved_trajectories(jsonl_path: str, k: int, output_dir: str):
    ...
    confidence intervals, and soft-token ratio statistics for OlympiadBench.
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for line in tqdm(f, desc="Evaluating Metrics", dynamic_ncols=True):
            ...
            for sample in data['samples']:
                ...
                parsed_true = parse(true_answer)
                parsed_pred = parse(extracted)
                is_correct = verify(parsed_pred, parsed_true)
    ...
    print("\n" + "=" * 50)
    print("TARPO OlympiadBench EVALUATION SUMMARY")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="data/olympiad/olympiad_bench.json", help="Path to OlympiadBench JSON")
    ...
    eval_dir = f"{checkpoint_path}/eval_olympiad_k{args.k}_batch{args.batch_size}"
