...
class GenerationMixin:
    ...
    def generate(
        ...
        use_mixed_rollout: bool = False,  
        action_temperature: float = 0.0, 
        soft_top_k: int = 10, 
        return_soft_metrics: bool = False,
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result = self._sample(
                ...
                use_mixed_rollout=use_mixed_rollout,
                action_temperature=action_temperature,
                soft_top_k=soft_top_k,
                return_soft_metrics=return_soft_metrics,
                is_greedy=is_greedy,
                **model_kwargs,
            )
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        actions_history = [] 
        mixed_embeds_history = []
        next_step_mixed_embeds = None
        is_thinking_tensor = torch.ones(batch_size, 1, dtype=torch.bool, device=input_ids.device)
        soft_topk_ids_history = [] 
        soft_topk_weights_history = []
        topk_entropy_history = [] 
        embed_dist_history = [] 
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {}) # TODO：这是llm最后一层tf的输出吗，这是bool

            model_inputs.update({"mixed_input_embeds": next_step_mixed_embeds} if next_step_mixed_embeds is not None else {})
            ...
            current_action = torch.zeros(batch_size, 1, dtype=torch.long, device=input_ids.device)

            if use_mixed_rollout:
                sampled_action = torch.zeros(batch_size, 1, dtype=torch.long, device=input_ids.device) 

                if hasattr(outputs, "action_logits"):
                    action_logits = outputs.action_logits[:, -1, :]

                    if action_temperature > 0.0: 
                        action_logits = action_logits / max(float(action_temperature), 1e-6)
                        
                    action_probs = nn.functional.softmax(action_logits, dim=-1)

                    hard_prob_fallback = torch.zeros_like(action_probs)
                    hard_prob_fallback[:, 0] = 1.0
                    action_probs = torch.where(is_thinking_tensor, action_probs, hard_prob_fallback)
                    
                    if is_greedy:
                        sampled_action = torch.argmax(action_probs, dim=-1, keepdim=True)
                    else:
                        sampled_action = torch.multinomial(action_probs, num_samples=1)

                action_probs_history.append(action_probs.unsqueeze(1))
                
                current_action = torch.where(is_thinking_tensor, sampled_action, torch.zeros_like(sampled_action))
                
                actions_history.append(current_action)
                is_soft_mask = (current_action == 1)

                if is_greedy: 
                    hard_token_ids = torch.argmax(probs, dim=-1)
                else:
                    hard_token_ids = torch.multinomial(probs, num_samples=1).squeeze(1)

                input_embeds_layer = self.get_input_embeddings()
                hard_embeds = input_embeds_layer(hard_token_ids).unsqueeze(1)

                top_k_probs, top_k_ids = torch.topk(probs, k=soft_top_k, dim=-1) 
                top_k_embeds = input_embeds_layer(top_k_ids) 
                top_k_probs_normalized = top_k_probs / top_k_probs.sum(dim=-1, keepdim=True)
                soft_embeds_sum = (top_k_probs.unsqueeze(-1) * top_k_embeds).sum(dim=1)
                soft_embeds = soft_embeds_sum.unsqueeze(1) 

                soft_topk_ids_history.append(top_k_ids.unsqueeze(1))
                soft_topk_weights_history.append(top_k_probs_normalized.unsqueeze(1))

                step_entropy = -torch.sum(top_k_probs_normalized * torch.log(top_k_probs_normalized + 1e-9), dim=-1)
                topk_entropy_history.append(step_entropy.unsqueeze(1)) 
                step_dist = torch.norm(hard_embeds - soft_embeds, p=2, dim=-1)
                embed_dist_history.append(step_dist)

                mask_broadcast = is_soft_mask.unsqueeze(-1).expand_as(hard_embeds) 
                next_step_mixed_embeds = torch.where(mask_broadcast, soft_embeds, hard_embeds)
                mixed_embeds_history.append(next_step_mixed_embeds)
                next_tokens = hard_token_ids
                ...
                next_step_mixed_embeds = None 
                mixed_embeds_history.append(None)
                topk_entropy_history.append(torch.zeros(batch_size, 1, device=input_ids.device))
                embed_dist_history.append(torch.zeros(batch_size, 1, device=input_ids.device))
                action_probs_history.append(torch.zeros(batch_size, 1, 2, device=input_ids.device))
            ...
            is_thinking_tensor = torch.tensor(is_thinking, dtype=torch.bool, device=input_ids.device).unsqueeze(1)
        ...
        if return_dict_in_generate:
            ...
            if use_mixed_rollout:
                use_soft_input_mask = torch.cat(actions_history, dim=1)
                mixed_input_embeds = torch.cat(mixed_embeds_history, dim=1)
                
                soft_topk_ids_tensor = torch.cat(soft_topk_ids_history, dim=1) 
                soft_topk_weights_tensor = torch.cat(soft_topk_weights_history, dim=1) 

                topk_entropy_tensor = torch.cat(topk_entropy_history, dim=1)
                embed_dist_tensor = torch.cat(embed_dist_history, dim=1) 

                batch_plain_texts = []
                batch_html_texts = []

                completion_ids = input_ids[:, input_len:]

                tokens_list = completion_ids.tolist()
                soft_mask_list = use_soft_input_mask.tolist()

                for i in range(batch_size):
                    tokens = tokens_list[i]
                    soft_mask = soft_mask_list[i]
                    plain_text_parts = []
                    html_text_parts = []
                    
                    if len(tokens) == 0:
                        batch_plain_texts.append("")
                        batch_html_texts.append("")
                        continue

                    current_mode = soft_mask[0]
                    current_chunk = [tokens[0]]

                    def process_chunk(chunk_ids, is_soft):
                        decoded_chunk = processing_class.decode(chunk_ids, skip_special_tokens=True)
                        if not decoded_chunk:
                            return "", ""

                        safe_chunk = decoded_chunk.replace("<", "&lt;").replace(">", "&gt;")
                        
                        if is_soft:
                            p_text = f"[{decoded_chunk}]"
                            h_text = f"<span style='background-color: #ffe066; font-weight: bold; border-radius: 3px; padding: 0 2px;'>[{safe_chunk}]</span>"
                        else:
                            p_text = decoded_chunk
                            h_text = safe_chunk
                        return p_text, h_text

                    for t_id, is_soft in zip(tokens[1:], soft_mask[1:]):
                        if is_soft == current_mode:
                            current_chunk.append(t_id) 
                        else:
                            p_text, h_text = process_chunk(current_chunk, current_mode)
                            plain_text_parts.append(p_text)
                            html_text_parts.append(h_text)
                            
                            current_mode = is_soft
                            current_chunk = [t_id]
                    ...
                    batch_plain_texts.append("".join(plain_text_parts))
                    batch_html_texts.append("".join(html_text_parts))

                if return_soft_metrics: 
                    return input_ids, use_soft_input_mask, mixed_input_embeds, soft_topk_ids_tensor, soft_topk_weights_tensor, batch_plain_texts, batch_html_texts, topk_entropy_tensor, embed_dist_tensor 
                else:
                    action_probs_tensor = torch.cat(action_probs_history, dim=1)
                    return input_ids, use_soft_input_mask, batch_plain_texts, action_probs_tensor 
            else:
                return input_ids
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            outputs = self(**model_inputs) # [local]
