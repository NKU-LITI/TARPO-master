...
def LlamaModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    use_soft_input_mask = kwargs.get('use_soft_input_mask', None)
    soft_topk_ids = kwargs.get('soft_topk_ids', None)
    soft_topk_weights = kwargs.get('soft_topk_weights', None)

    if soft_topk_ids is not None and use_soft_input_mask is not None:
        c_len = soft_topk_ids.shape[1]
        live_topk_embeds = self.embed_tokens(soft_topk_ids) 
        live_weights = soft_topk_weights.to(live_topk_embeds.dtype).unsqueeze(-1)
        live_soft_embeds = (live_weights * live_topk_embeds).sum(dim=2)
        new_inputs_embeds = inputs_embeds.clone()
        mask_bool = use_soft_input_mask.to(torch.bool)
        completion_embeds = new_inputs_embeds[:, -c_len:].clone()
        completion_embeds[mask_bool] = live_soft_embeds[mask_bool].to(completion_embeds.dtype)
        new_inputs_embeds[:, -c_len:] = completion_embeds
...
def LlamaModel_fast_forward_inference(
    ...
    *args, **kwargs,
    ...
    mixed_input_embeds = kwargs.get("mixed_input_embeds", None)
    if mixed_input_embeds is not None:
        mixed_input_embeds = mixed_input_embeds[:,:self.max_seq_length,:] # [8, 1, 1536]
        X = mixed_input_embeds.to(dtype=_get_dtype(self.config.torch_dtype))
    else:
        X = self.model.embed_tokens(input_ids)
        X = X.to(_get_dtype(self.config.torch_dtype))


    bsz, q_len, hd = X.shape # [8, 1, 1536]
    assert(q_len == 1)
        ...
        *args, **kwargs,
        ...
        if hasattr(self, "action_head") and self.action_head is not None:
            if num_logits_to_keep != 0:
                action_logits = self.action_head(hidden_states[:, -num_logits_to_keep:, :].to(dtype))
            else:
                action_logits = self.action_head(hidden_states.to(dtype))

            action_logits = action_logits.to(_get_dtype(self.config.torch_dtype))
        ...
        **kwargs,
        ...
        if train_action_head:
            ...
            try:
                ...
                new_dtype = model.get_input_embeddings().modules_to_save.default.weight.dtype   
            # PEFT 会把 target 包裹在 modules_to_save 中
            if hasattr(model.base_model.model, "action_head"):
                action_module = model.base_model.model.action_head
                if hasattr(action_module, "modules_to_save"):
                    action_module.modules_to_save.default.to(device="cuda", dtype=new_dtype, non_blocking=True)
                    action_module.modules_to_save.default.requires_grad_(True)
                else:
                    action_module.to(device="cuda", dtype=new_dtype, non_blocking=True)
